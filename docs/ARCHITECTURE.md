...
| postgres | postgres:18-alpine | 127.0.0.1:5432 | データベース |
| redis | redis:7-alpine | 127.0.0.1:6379 | キャッシュ / ジョブキュー |
| e2e | カスタム (Node 20 + Playwright) | — | E2E テスト (オンデマンド起動) |
...
| コンポーネント | 選択 | 理由 |
|-------------|------|------|
| コンピュート | ECS Fargate (Spot) | コンテナ管理不要、タスク数 0 で停止可能、学習価値が高い, Spot で最大 70% コスト削減（学習用途のため中断許容） |
...
NAT Instance (t4g.nano) を Public Subnet に配置し、Private Subnet からのインターネットアクセスを提供（NAT Gateway の代替、コスト最適化）。

---

## 4. データベース設計方針

- RDBMS: PostgreSQL 18（Rails との親和性、JSON サポート、実績）
...
| Angular | 19.x+ | package-lock.json で固定 |
| PostgreSQL | 18 | Docker image tag で固定 |
