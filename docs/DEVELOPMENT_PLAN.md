...
| 認証コントローラー | `api/app/controllers/api/v1/sessions_controller.rb` |
...
| 請求書生成サービス (バッチ) | `api/app/services/invoice_generation/batch_service.rb` |
| 請求書生成サービス (個別) | `api/app/services/invoices/generate_service.rb` |
...
| ページネーション | kaminari | 一覧表示のページネーション |
