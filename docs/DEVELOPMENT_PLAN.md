...
| 1-8 | ヘルスチェックエンドポイント | `feat/health-check` | `GET /api/health` |
...
| FEAT-0041 | 通知 | 3E | FEAT-0030 | 請求書発行通知、入金期限通知。ログ出力で代替 |

### 5.2 API 仕様 (API)

| ID | タイトル | 関連 FEAT | エンドポイント概要 |
|----|---------|-----------|-------------------|
| API-0001 | 認証 API | FEAT-0001 | `POST /auth/signup`, `POST /auth/login`, `POST /auth/refresh`, `DELETE /auth/logout` |
| API-0002 | ユーザー管理 API | FEAT-0001, FEAT-0002 | `GET/PATCH /users/me`, `GET/POST/PATCH/DELETE /admin/users` |
| API-0003 | 顧客 API | FEAT-0010, FEAT-0011 | `GET/POST/PATCH/DELETE /customers`, `GET /customers/search` |
| API-0004 | 契約 API | FEAT-0020 | `GET/POST/PATCH /contracts`, `POST /contracts/:id/activate` 等の状態遷移 |
| API-0005 | 契約明細 API | FEAT-0021 | `GET/POST/PATCH/DELETE /contracts/:id/line_items` |
| API-0006 | 使用量記録 API | FEAT-0022 | `POST/GET /contracts/:id/usages` |
| API-0007 | 請求書 API | FEAT-0030, FEAT-0031 | `GET /invoices`, `GET /invoices/:id`, `POST /invoices/generate` |
| API-0008 | 入金 API | FEAT-0032, FEAT-0033 | `GET/POST /payments`, `POST /payments/:id/reconcile` |
...
| contracts | customer_id, status, start_date, end_date | 状態遷移管理 |
...
| 成果物 | 主要ファイル |
|--------|------------|
| Customer モデル + マイグレーション | `api/app/models/customer.rb` |
| 顧客コントローラー | `api/app/controllers/api/v1/customers_controller.rb` |
| 顧客シリアライザ | `api/app/serializers/customer_serializer.rb` |
| Angular 顧客サービス | `web/src/app/features/customers/services/customer.service.ts` |
| 顧客一覧コンポーネント | `web/src/app/features/customers/customer-list/` |
| 顧客詳細コンポーネント | `web/src/app/features/customers/customer-detail/` |
...
| 通知モデル + サービス | `api/app/models/notification.rb`, `api/app/services/notification_service.rb` |
...
| 入金消込の部分消込/過入金処理 | Medium | Medium | 仕様で全パターン (全額/一部/過入金) を列挙。金額は BigDecimal で扱う |
...
| 金額計算の精度 | 浮動小数点を使わない。Rails では BigDecimal、DBでは decimal 型 |
...
| 金額計算 | BigDecimal (Ruby) / decimal (PostgreSQL) | 浮動小数点誤差を回避 |
