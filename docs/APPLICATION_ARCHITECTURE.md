...
Rails.application.routes.draw do
  namespace :api do
    namespace :v1 do
      ...
      resources :usage_records, only: %i[index show create]
...
class InvoiceSerializer
  ...
  # 金額は integer (cents) で保存し、シリアライズ時に文字列に変換
  attribute :subtotal_amount do |invoice|
    format_amount(invoice.subtotal_amount)
  end
...
export class ContractFormComponent implements OnInit {
  ...
  form = this.fb.group({
    ...
    contractNumber: [
      "",
      [Validators.required, Validators.pattern(/^CON-\d{6}$/)],
    ],
...
| role | varchar(20) | NOT NULL | admin / accountant / sales |
...
|--------|-----|------|------|
| id | bigint | PK, NOT NULL | |
| code | varchar(20) | NOT NULL, UNIQUE | 顧客コード (例: CLT-000001) |
...
|--------|-----|------|------|
| id | bigint | PK, NOT NULL | |
| contract_number | varchar(20) | NOT NULL, UNIQUE | 契約番号 (例: CON-000001) |
...
- `index_contracts_on_status_and_billing_cycle` (月次バッチ用の複合インデックス)
...
|--------|-----|------|------|
| id | bigint | PK, NOT NULL | |
| invoice_number | varchar(20) | NOT NULL, UNIQUE | 請求書番号 (例: INV-202604-0001) |
| contract_id | bigint | FK(contracts), NOT NULL | 契約 |
...
| idempotency_key | varchar(100) | NOT NULL, UNIQUE | べき等性キー |
...
- `index_invoices_on_contract_id_and_billing_period_start` (重複防止用)
...
| 候補 | 長所 | 短所 |
|------|------|------|
| **integer (cents)** | 浮動小数点誤差なし、計算が高速、比較が正確 | 表示時に 100 で割る変換が必要 |
| decimal(15,2) | 直感的な値 | DB 間の挙動差、Rails の BigDecimal 変換コスト |

- cents で保存し、シリアライザで `"1234.56"` のように文字列に変換して返す。
- フロントエンドでの浮動小数点誤差を完全に排除する。
- 通貨計算では **Banker's Rounding** (四捨五入ではなく偶数丸め) を使用する。
...
| メソッド | パス | 認証 | 認可 | 説明 |
|---------|------|------|------|------|
| POST | /api/v1/session | 不要 | - | ログイン |
| DELETE | /api/v1/session | 必要 | 全ロール | ログアウト |
| POST | /api/v1/session/refresh | Cookie | - | トークンリフレッシュ |
...
| メソッド | パス | 認証 | 認可 | クエリパラメータ | 説明 |
|---------|------|------|------|----------------|------|
| GET | /api/v1/contracts | 必要 | 全ロール | `client_id`, `status`, `billing_cycle`, `sort`, `direction`, `page`, `per_page` | 一覧取得 |
...
| POST | /api/v1/contracts/:id/activate | 必要 | admin | - | 有効化 |
| POST | /api/v1/contracts/:id/suspend | 必要 | admin | - | 一時停止 |
| POST | /api/v1/contracts/:id/resume | 必要 | admin | - | 再開 |
| POST | /api/v1/contracts/:id/terminate | 必要 | admin | - | 解約 |
...
| POST | /api/v1/contracts/:contract_id/contract_items | 必要 | admin, sales | 作成 |
| GET | /api/v1/contract_items/:id | 必要 | 全ロール | 詳細取得 |
| PATCH | /api/v1/contract_items/:id | 必要 | admin, sales | 更新 |
| DELETE | /api/v1/contract_items/:id | 必要 | admin | 削除 (draft 契約のみ) |
...
| POST | /api/v1/invoices/:id/approve | 必要 | admin, accountant | - | 承認 |
| POST | /api/v1/invoices/:id/send_invoice | 必要 | admin, accountant | - | 送付済みに変更 |
...
| メソッド | パス | 認証 | 認可 | クエリパラメータ | 説明 |
|---------|------|------|------|----------------|------|
| GET | /api/v1/usage_records | 必要 | 全ロール | `contract_item_id`, `recorded_date_from`, `recorded_date_to`, `sort`, `direction`, `page`, `per_page` | 一覧取得 |
| POST | /api/v1/usage_records | 必要 | admin, sales | - | 使用量登録 |
...
│       |                                                         │
│       ├── 1. 対象契約の抽出                                       │
│       │     active 状態 かつ billing_cycle に該当する契約           │
│       │                                                         │
...
module InvoiceGeneration
  class BatchService
    ...
    def target_contracts
      Contract
        .where(status: :active)
        .where(billing_cycle: billing_cycle_for_period)
    ...
    def billing_cycle_for_period
      # 月次は毎月、四半期は 1,4,7,10 月、年次は 1 月
      cycles = [:monthly]
      cycles << :quarterly if [1, 4, 7, 10].include?(billing_period.month)
      cycles << :yearly if billing_period.month == 1
      cycles
    end
  ...
  class GenerateService
    ...
    def build_invoice
      Invoice.new(
        ...
        due_date: @billing_period.end_of_month + @contract.payment_term_days.days,
        idempotency_key: @idempotency_key
      )
    end
    ...
    def generate_items(invoice)
      @contract.contract_items.each do |item|
        ...
        amount_cents = calculator.calculate_amount(item, quantity)
        tax_amount_cents = (amount_cents * item.tax_rate).round
    ...
    def calculate_totals(invoice)
      invoice.subtotal_amount_cents = invoice.invoice_items.sum(&:amount_cents)
      invoice.tax_amount_cents = invoice.invoice_items.sum(&:tax_amount_cents)
      invoice.total_amount_cents = invoice.subtotal_amount_cents + invoice.tax_amount_cents
    end
...
module InvoiceGeneration
  class FixedCalculator
    ...
    def calculate_amount(contract_item, quantity)
      (contract_item.unit_price_cents * quantity).round
    end
  end
end
...
module InvoiceGeneration
  class UsageCalculator
    def calculate_quantity(contract_item, billing_period)
      UsageRecord
        .where(contract_item:)
        .where(recorded_date: billing_period..billing_period.end_of_month)
        .sum(:quantity)
    end
...
| リソース | admin | accountant | sales | viewer |
|---------|-------|-----------|-------|--------|
| Client CRUD | 全て | 閲覧 | 作成/編集/閲覧 | 閲覧 |
| Contract CRUD | 全て | 閲覧 | 作成/編集/閲覧 | 閲覧 |
| Contract 状態遷移 | 全て | - | - | - |
| Invoice 閲覧 | 全て | 全て | 閲覧 | 閲覧 |
| Invoice 承認/送付 | 全て | 承認/送付 | - | - |
| Invoice 無効化 | 全て | - | - | - |
| Payment | 全て | 記録/閲覧 | - | - |
| UsageRecord | 全て | 閲覧 | 作成/閲覧 | 閲覧 |
| User 管理 | 全て | - | - | - |
...
Rack::Attack.throttle("api/v1/session", limit: 5, period: 60) do |req|
