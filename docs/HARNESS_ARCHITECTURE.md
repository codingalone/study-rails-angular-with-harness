...
> **⚠ 本一覧は全 Phase 完了時の目標構成であり、現行の CI_CD.md とは差分がある。** 現行 CI_CD.md の Required Status Checks は lint, test, security のみ。Phase 2 で追加: Angular SAST、strong_migrations、Actions SHA ピン留め、`paths-filter` によるジョブ絞り込み（CI_CD.md は PR を「フル CI」と記載しているが、本設計では `docs/**` 変更時はドキュメント lint のみ実行）。Phase 3 で追加: committee（OpenAPI スキーマ整合性チェック）、レビューコメント存在チェック（マルチエージェントレビュー導入後に CI 自動化）。CI_CD.md の更新は各 Phase の最初のタスクとして実施する。
