...
> **注**: 本セクションは Phase 0 から即時適用する運用プロセスである。ただし、「レビューコメント存在チェック」の CI 自動化（L3 ゲート）は Phase 3 でマルチエージェントレビュー導入後に実装する。Phase 2 までは手動で本プロセスに従う。現行の DEVELOPMENT_GUIDE.md / CI_CD.md の「CI 全通過のみがマージ条件」に加え、本プロセスによる AI レビュー PASS を追加のマージ条件とする。
