...
> **⚠ 本一覧は全 Phase 完了時の目標構成であり、現行の CI_CD.md とは差分がある。** 現行 CI_CD.md の Required Status Checks は lint, test, security + AI クロスレビュー PASS（§7.4 参照）。Phase 2 で追加予定: Angular SAST、strong_migrations、Actions SHA ピン留め、`paths-filter` によるジョブ絞り込み（CI_CD.md は PR を「フル CI」と記載しているが、本設計では `docs/**` 変更時はドキュメント lint のみ実行）。Phase 3 で追加予定: committee（OpenAPI スキーマ整合性チェック）、レビューコメント存在チェック（マルチエージェントレビュー導入後に CI 自動化）。CI_CD.md の更新は各 Phase の最初のタスクとして実施する。
...
> **注**: 本セクションは Phase 0 から即時適用する運用プロセスである。ただし、「レビューコメント存在チェック」の CI 自動化（L3 ゲート）は Phase 3 でマルチエージェントレビュー導入後に実装する。Phase 2 までは手動で本プロセスに従う。DEVELOPMENT_GUIDE.md / CI_CD.md のマージ条件（CI 全通過 + AI クロスレビュー PASS）の一部として、本プロセスが AI レビューの実施手順を定義する。
...
L3 CI ゲート層: レビューコメント存在チェック（Phase 3 で CI に統合）
```

> **Phase 3 での CI 統合**: GitHub Actions でマージ前に「レビュー完了サマリー」コメントの存在を自動チェックする（マルチエージェントレビュー導入後）。これにより「レビューなしマージ」を構造的に不可能にする。
...
- [x] `.claude/commands/` にレビュースキル定義（review-codex.md, review-gemini.md）
...
- [ ] Dependabot 設定
- [ ] PR テンプレート + 自動チェックリスト
- [ ] CI_CD.md の更新（本ドキュメントの Required Status Checks と整合）

### Phase 3: アプリケーション実装

- [ ] /spec-draft スキルの活用開始
- [ ] /implement スキルの活用開始
- [ ] OpenAPI スキーマ整合性チェック導入（committee gem — CI_CD.md への反映含む）
- [ ] マルチエージェントレビューの CI 自動化（手動プロセスは Phase 0 から適用済み）
- [ ] レビューコメント存在チェックの CI ゲート化
...
| マルチエージェントレビューの CI 自動化を Phase 3 で実施 | 手動プロセスは Phase 0 から適用。L1-L3 基盤を固めてから L3 ゲートとして自動化 |
