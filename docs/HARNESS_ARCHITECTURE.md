...
| git reset --hard | PreToolUse: Bash で git reset --hard | 破壊的 Git 操作の防止（DEVELOPMENT_GUIDE.md §10 参照） |
...
> **注**: 本一覧は全 Phase 完了時の目標構成。CI_CD.md は Phase 2 で本一覧と整合するよう更新する。以下の差分が CI_CD.md にまだ反映されていないため、Phase 2 で追加する: Angular SAST、committee、strong_migrations、Actions SHA ピン留め、および `docs/**` 変更時のドキュメント lint のみ実行（CI_CD.md は PR を「フル CI」と記載しているが、`paths-filter` で変更対象に応じたジョブ絞り込みを行う設計）。
