...
プロジェクトルートの CLAUDE.md が最上位のコントロールパネル。OpenAI の実験では巨大な指示書は逆効果であり、約 100 行の「地図」として機能させ、詳細は `docs/` への参照で補う。
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file_path=$(echo \"$CLAUDE_TOOL_INPUT\" | jq -r '.file_path // empty') && if echo \"$file_path\" | grep -qE '\\.rb$'; then docker compose exec -T api bundle exec rubocop --autocorrect-all --fail-level E \"$file_path\" 2>&1 | tail -5; fi",
            "timeout": 30
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file_path=$(echo \"$CLAUDE_TOOL_INPUT\" | jq -r '.file_path // empty') && if echo \"$file_path\" | grep -qE '\\.ts$'; then docker compose exec -T web npx ng lint 2>&1 | tail -5; fi",
            "timeout": 30
          }
        ]
      }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file_path=$(echo \"$CLAUDE_TOOL_INPUT\" | jq -r '.file_path // empty') && if echo \"$file_path\" | grep -qE '(\\.rubocop\\.yml|\\.eslintrc|eslint\\.config|biome\\.json|\\.tflint\\.hcl)$'; then echo 'BLOCKED: リンター/IaC設定の変更にはユーザー承認が必要です' >&2 && exit 2; fi"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file_path=$(echo \"$CLAUDE_TOOL_INPUT\" | jq -r '.file_path // empty') && if echo \"$file_path\" | grep -qE '(docker-compose\\.yml|Dockerfile|config/initializers/cors\\.rb)$'; then echo 'BLOCKED: Docker構成/セキュリティ設定の変更にはユーザー承認が必要です' >&2 && exit 2; fi"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file_path=$(echo \"$CLAUDE_TOOL_INPUT\" | jq -r '.file_path // empty') && if echo \"$file_path\" | grep -qE '(Gemfile|package\\.json)$'; then echo 'BLOCKED: パッケージ追加にはユーザー承認が必要です' >&2 && exit 2; fi"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "if": "Bash(rm *)",
            "command": "echo 'BLOCKED: ファイル削除にはユーザー承認が必要です' >&2 && exit 2"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            ...
            "command": "echo 'BLOCKED: force push は禁止です' >&2 && exit 2"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            ...
            "command": "echo 'BLOCKED: git reset --hard にはユーザー承認が必要です' >&2 && exit 2"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "if": "Bash(*rails generate migration*)",
            "command": "echo 'BLOCKED: DB マイグレーション新規作成にはユーザー承認が必要です' >&2 && exit 2"
          }
        ]
      }
    ]
...
DEVELOPMENT_GUIDE.md に記載の通り、pre-commit hook は `~/dotfiles/hooks/pre-commit` からコピーして `.git/hooks/` にインストールする。
...
> **Lefthook について**: 母艦への変更禁止の原則（ARCHITECTURE.md）と Lefthook のホスト側インストール要件の間に実装上の矛盾がある。Phase 1 でこの問題を解決する方式を ADR として記録する。選択肢: (a) Docker コンテナ内に Lefthook をインストールし entrypoint でフックを登録、(b) pre-commit hook スクリプト内で `docker compose exec` を直接呼び出す、(c) Claude Code hooks で代替する。
...
| MUST ASK 項目 | フック実装 |
|---------------|-----------|
| リンター設定の変更 | PreToolUse: Edit\|Write でファイルパスチェック |
| Docker 構成変更 | PreToolUse: Edit\|Write で docker-compose.yml, Dockerfile |
| セキュリティ設定変更 | PreToolUse: Edit\|Write で cors.rb, CSP 設定 |
| 新規パッケージ追加 | PreToolUse: Edit\|Write で Gemfile, package.json |
| ファイルの削除 | PreToolUse: Bash で rm コマンド |
| DB マイグレーション作成 | PreToolUse: Bash で rails generate migration |
| force push | PreToolUse: Bash で git push --force |
| git reset --hard | PreToolUse: Bash で git reset --hard |
...
| E2E テスト | Playwright (main push 時のみ) | 失敗 0 | Yes |
| Rails セキュリティ | Brakeman | Warning 以上 0 | Yes |
...
- [ ] Docker Compose 構成
- [ ] pre-commit hook（秘密情報スキャン — DEVELOPMENT_GUIDE.md の手順に準拠）
...
| CLAUDE.md を約 100 行に維持 | OpenAI の教訓: 地図であり百科事典ではない |
| Brakeman 閾値を L2/L3 とも Warning 以上で統一 | 多層防御では上層が下層と同等以上に厳格であるべき |
